...
class Bot {
  ...
  fsm2(cmd) {
    ...
    switch (this.state) {
      ...
      case 'spiral':
        if (this.sns.deltaEnergy >= 5) this.transitionTo('spiral');
        if (this.counter > 140) this.transitionTo('aggressive');
        break;
    }
  }
  ...
  aggressive() {
    ...
    this.mtr.left = 5 + 50 * this.sns.right;
    this.mtr.right = 5 + 50 * this.sns.left;
  }
  
  spiral() {
    this.mtr.left = 5;
    this.mtr.right = 0.3 * sqrt(this.counter);
  }
