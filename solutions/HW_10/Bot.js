...
class Bot {
  ...
  updateEstimates() {
    ...
    let eps = 0.15; // learning rate
    let reward = this.sns.deltaEnergy;
    if(reward == 0) return;
    if(this.sns.lastColorConsumed == "red") {
      this.estimatedValue[0] += eps * (reward - this.estimatedValue[0]);
    } else if (this.sns.lastColorConsumed == "green" ) {
      this.estimatedValue[1] += eps * (reward - this.estimatedValue[1]);
    } else if (this.sns.lastColorConsumed == "blue" ) {
      this.estimatedValue[2] += eps * (reward - this.estimatedValue[2]);
    }
  ...
  seekUser() {
    ...
    let best = max(this.estimatedValue);
    if(best < 1.0) this.seekAll();
    else if (this.estimatedValue[0] == best)this.seekRed();
    else if (this.estimatedValue[1] == best)this.seekGreen();
    else if (this.estimatedValue[2] == best)this.seekBlue();
  }
