class Bot {
  ...
  update() {
    ...
    // implement wrapped boundary conditions here
  ...
  controller() {
    ...
    switch (select("#controller").value()) {
      ...
      case 'orthokinesisPos':
        ...
        this.speed = (this.tsense > 0.15) ? 1 : 0;
        this.turnAngle = 0.1 * random(-1, 1);
        break;
      case 'orthokinesisNeg':
        // bot speed decreases with temperature
        this.speed = (this.tsense < 0.85) ? 1 : 0;
        this.turnAngle = 0.1 * random(-1, 1);
        break;
      case 'run-tumble':
        this.speed = 1;
        this.turnAngle = (this.tsense > this.memory) ? 0 : random(-PI, PI);
        this.memory = this.tsense;
        break;
    }
  }
