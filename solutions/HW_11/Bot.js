...
class Bot {
  ...
  handCoded() {
    ...
    if (snsFWD == obj.good) {
      ...
      if(snsLEFT == obj.bad || snsLEFT == obj.wall) this.action = RIGHT;
      else if(snsRIGHT == obj.bad || snsRIGHT == obj.wall) this.action = LEFT;
      else this.action = random([LEFT, RIGHT]);
    } else if (snsLEFT == obj.wall) {
      if(random() < 0.1) this.action = RIGHT;
      else this.action = FWD;
    } else if (snsRIGHT == obj.wall) {
      if(random() < 0.1) this.action = LEFT;
      else this.action = FWD;
    } else {
      if(random() < 0.98) this.action = FWD;
      else this.action = random([LEFT, RIGHT]);
    }
  }
  //===================================
